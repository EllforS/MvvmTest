...
object NotificationUtil {
     ...
     * @return 是否已开启通知权限
     */
    fun requsetNotificationPermission(context: Context): Boolean {
        return if (!isNotificationEnabled(context) && context is Activity) {
            AlertDialog.Builder(context)
                .setTitle("提示")
                .setMessage("请开启通知权限")
                .setPositiveButton("去开启") { _, _ -> openNotify(context) }
    ...
    private fun isNotificationEnabled(context: Context): Boolean {
        ...
        try {
            ...
            val checkOpNoThrowMethod: Method =
